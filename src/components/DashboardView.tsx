...
export default function DashboardView({ tasks, settings, go }: { tasks: Task[]; settings: UserSettings; go: (p: Page) => void }) {
  ...
  return <>
    <div className="page-title"><small>{formatDate(now).toUpperCase()}</small><h1>Halo{settings.name ? `, ${settings.name.split(" ")[0]}` : ""}.</h1></div>
    ...
    <section className="panel"><div className="panel-head"><div><h2>Deadline terdekat</h2></div><button onClick={() => go("Tugas")}>Kelola tugas</button></div>{near.length ? near.map((task) => <article className="row" key={task.id}><i style={{ background: COURSE_SCHEDULE.find((course) => course.code === task.courseCode)?.color }} /><div className="row-title"><b>{task.title}</b><small>{courseName(task.courseCode)}</small></div><time>{new Date(task.dueAt).toLocaleString("id-ID")}</time></article>) : <div className="empty"><CheckSquare /><h3>Belum ada deadline</h3><button onClick={() => go("Tugas")}>+ Tambah tugas</button></div>}</section>
  </>;
}
