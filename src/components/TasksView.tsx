...
export default function TasksView({
  ...
  const [edit, setEdit] = useState<Task | null>(null),
    [filter, setFilter] = useState("aktif"),
    [deleting, setDeleting] = useState<Task | null>(null);
  const visible = tasks.filter(
    (t) =>
      filter === "semua" || (filter === "selesai" ? t.completed : !t.completed),
  );
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!edit) return;
    setTasks(
      tasks.some((x) => x.id === edit.id)
        ? tasks.map((x) => (x.id === edit.id ? edit : x))
        : [...tasks, edit],
    );
    setEdit(null);
  };
  return (
    <>
      ...
      <section className="panel">
        {visible.length ? (
          visible.map((t) => (
            <article className="row task-row" key={t.id}>
              <input
                type="checkbox"
                checked={t.completed}
                onChange={() =>
              ...
              <div>
                <b>{t.title}</b>
                <small>
                  {courseName(t.courseCode)}
                  {t.meetingNo ? ` · Pertemuan ${t.meetingNo}` : ""} ·{" "}
                  {t.description || "Tanpa catatan"}
                </small>
              </div>
              <time>
                {validDate(t.dueAt)
              ...
              <button className="icon" onClick={() => setEdit(t)}>
                <Pencil />
              </button>
              <button
      ...
      <ConfirmModal
        ...
        title="Hapus tugas?"
        message={`Tugas “${deleting?.title ?? ""}” akan dihapus permanen dan tidak dimunculkan kembali oleh sinkronisasi.`}
        onCancel={() => setDeleting(null)}
        onConfirm={() => {
          if (deleting) setTasks(tasks.filter((x) => x.id !== deleting.id));
          setDeleting(null);
        }}
      />
      {edit && (
        <Modal
          title={
            tasks.some((x) => x.id === edit.id) ? "Edit tugas" : "Tambah tugas"
          }
